...
public class SrcBuilder
{
    ...
    public int IndentSize { get => _size; set => SetIndent(value, _level); }

    /// <summary>Specifies the number of indentation levels from the beginning of the line.</summary>
    public int IndentLevel { get => _level; set => SetIndent(_size, value); }
...
#pragma warning restore CS1503,CA1830
    ...
    /// <returns></returns>
    public SrcBuilder DocLine(string elementName, string? attributes = null, string? content = null)
    {
        if (string.IsNullOrEmpty(attributes))
        {
            AppendIndent("/// <").Append(elementName);
        }
        else
        {
            AppendIndent("/// <").Append(elementName).Append(' ').Append(attributes);
        }

        if (!string.IsNullOrEmpty(content))
        {
            Append('>').Append(content).Append("</").Append(elementName).Append(">");
        }
        else
        {
            Append("/>");
